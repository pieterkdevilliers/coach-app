...
@router.post("/calls/{call_id}/scribe/submit", status_code=status.HTTP_202_ACCEPTED)
async def submit_to_scribe(
    call_id: UUID,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> dict:
    try:
        job_id = await scribe_service.submit(db, call_id, current_user.business_id)
